...
__global__ void naive_matmul_kernel(MatrixView<const float> A, MatrixView<const float> B, MatrixView<float> C) {
    ...
    const std::size_t row = std::size_t{blockIdx.x} * blockDim.x + threadIdx.x;
    const std::size_t col = std::size_t{blockIdx.y} * blockDim.y + threadIdx.y;
...
void launch_naive_kernel(MatrixView<const float> A, MatrixView<const float> B, MatrixView<float> C,
    ...
    // CUDA x covers matrix rows.
    const auto grid_x = static_cast<unsigned>(cuda::ceil_div(C.extent(0), std::size_t{topology.block.x}));
    const auto grid_y = static_cast<unsigned>(cuda::ceil_div(C.extent(1), std::size_t{topology.block.y}));
